      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            step="0.01"
            className="input"
            {...register('basePrice', { required: 'Required', valueAsNumber: true })}
          />
              ...
              <div key={rt.id} className="card p-5">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex gap-1">
                    <button className="btn-ghost h-8 w-8 p-0" onClick={() => openEdit(rt)}>
                      <Pencil className="h-3.5 w-3.5" />
                    </button>
                    <button className="btn-ghost h-8 w-8 p-0 text-red-600" onClick={() => setDeleteTarget(rt)}>
                ...
                <div className="mt-3 flex items-center justify-between text-sm">
                  <span className="text-slate-500">Up to {rt.capacity} guests</span>
                  <span className="font-semibold text-brand-800">{formatCurrency(rt.basePrice)}</span>
                </div>
              </div>
            ))}
          </div>
        ) : (
          <EmptyState title="No room types yet" description="Create your first room type to get started." />
        )}
      </div>
