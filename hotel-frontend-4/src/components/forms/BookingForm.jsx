...
export default function BookingForm({ room, onSubmit, loading }) {
  ...
  return (
    <form onSubmit={handleSubmit((values) => onSubmit({ ...values, roomId: room.id }))} className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          />
          {errors.checkInDate && <p className="error-text">{errors.checkInDate.message}</p>}
        </div>
        <div>
          ...
          />
          {errors.checkOutDate && <p className="error-text">{errors.checkOutDate.message}</p>}
        </div>
      </div>
      ...
      {nights > 0 && (
        <div className="rounded-lg bg-slate-50 px-4 py-3 text-sm text-slate-600">
          <div className="flex justify-between">
            <span>
              {formatCurrency(room?.basePrice)} &times; {nights} night{nights > 1 ? 's' : ''}
            </span>
